...
class ScheduleMoveInteractionTests(StaticLiveServerTestCase):
    ...
    def test_selects_linked_assignment_and_only_approved_destinations(self):
        ...
        self.assertTrue(
            self.cell("tue_am1").get_attribute("aria-label").startswith("Preview and move Ropes to")
        )
        ...
        self.assertIn(
            "Linked assignment with 2 blocks.",
            self.page.locator("[data-schedule-move-announcer]").text_content(),
        )
        ...
        self.assertEqual(
            self.page.locator("[data-schedule-move-announcer]").text_content(),
            "Schedule move selection canceled.",
        )
